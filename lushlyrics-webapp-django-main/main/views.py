...
def playlist(request):
    if playlist_user.objects.filter(username=request.user).count() == 0:
        return HttpResponseRedirect(
            reverse("main:default")
        )  # project does not have this funciontality so I just redirect to index for now
    cur_user = playlist_user.objects.get(username=request.user.username)
    try:
